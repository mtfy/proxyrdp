					...
					<div class="flex flex-col w-full bg-theme-background-alt rounded-[4px] w-full m-0 p-0 relative p-[30px] mx-0 my-[30px]">
						<div class="flex flex-col w-full relative">
							<div class="flex flex-col w-full relative justify-center items-center mb-5">
								<div class="flex flex-row relative space-x-2 items-center justify-center">
									<div class="flex flex-col leading-5 select-none pointer-events-none">
										<svg xmlns="http://www.w3.org/2000/svg" role="img" class="inline-flex text-theme-primary-600 select-none pointer-events-none leading-5" width="24px" height="24px" fill="currentColor" viewBox="0 0 16 16"><path d="M11 1a2 2 0 0 0-2 2v4a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2h5V3a3 3 0 0 1 6 0v4a.5.5 0 0 1-1 0V3a2 2 0 0 0-2-2zM3 8a1 1 0 0 0-1 1v5a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V9a1 1 0 0 0-1-1H3z"/></svg>
									</div>
									<div class="flex flex-col">
										<h1 class="inline-flex text-theme-primary-600 font-motify font-bold capitalize whitespace-pre-wrap leading-5 text-[18px] md:text-[24px]">Account recovery</h1>
									</div>
								</div>
							</div>
							...
							<form class="flex flex-col space-y-[20px] w-full relative" @submit.prevent="submit">
								...
								<div class="flex flex-col w-full m-0 p-0 pt-[12px]">
									<Button :type="'submit'" :custom-class="'bg-theme-secondary-500 text-white w-full capitalize text-center justify-center items-center text-[12px] md:text-[14px] hover:bg-theme-secondary-700 hover:text-[#F5F5F5]'">Recover my account</Button>
								</div>
							</form>
...
<script setup>
	...
	const submit = () => {
		form.post(route('clientarea.login.recover'), {
			onFinish: () => {
				form.reset('password')
			},
			onSuccess: () => {
				Swal.fire({
					icon: 'info',
					text: `If the email address exists in our database, we have emailed you a link to reset your password!`
				});
			},
			onError: (errors) => {
